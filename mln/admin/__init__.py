...
from ..models.static import Answer, ArcadePrize, BlueprintInfo, BlueprintRequirement, ItemInfo, ItemType, MessageBody, ModuleEditorType, ModuleExecutionCost, ModuleInfo, ModuleSetupCost, ModuleYieldInfo, NetworkerFriendshipConditionDev, NetworkerFriendshipConditionSourceDev, NetworkerMessageTriggerDev, NetworkerMessageAttachmentDev, StartingStack, Question
...
has_trigger = lambda obj: NetworkerMessageTriggerDev.objects.filter(body=obj).exists() or NetworkerFriendshipConditionDev.objects.filter(Q(success_body=obj) | Q(failure_body=obj)).exists()
...
friend_cond_admin = make_inline(NetworkerFriendshipConditionDev, NetworkerFriendshipConditionSourceDev)
